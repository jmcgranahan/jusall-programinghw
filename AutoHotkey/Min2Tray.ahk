...
;
; _requires ms windows nt/2000/xp or newer.
;  (not tested under ms vista, yet.)
;
...
;   Demokos and Andreone (French translation)
...
Loop, %h_ID0%
{
   ...
   Loop
   {
      ...
      newWinID := f_SubstEvilClass( h_WinID, h_Class, h_Application )
      If ( h_WinID = newWinID )
   ...
   If ( Not h_JTA Or retVal = "m2t" ) {
      ...
      If ( m2t_NoMinGlobal )
         h_NoMin := TRUE
      Else {
         ; read out app|class|nm for "nominimize"
         h_NoMin := FALSE
...
Sleep, 50
If ( h_NoMin )
   ; use ALT+ESC if last hidden window did NOT use task bar (WinMinimize)
   SendInput, !{ESC}
Else
   WinActivateBottom, A
...
h_SetTimedCheck:
   ; get interval from registry, if any
	...
	If ( ErrorLevel OR tmp < 200 OR tmp > 1000000 )
		tmp = 2500
		      ...
		      If ( tmpNew <> h_Title )
		      {
               If ( h_sotcEnabled )
                  If ( RegExMatch( tmpNew, h_sotcMatch ) ; show window after
                     ...
                     Gosub, h_Unhide
		         Else
		         {  ; update window title and redraw menu
		            h_Title = %tmpNew%
		            Gosub, h_SetMenu
...
h_SetTray:
   h_ciEnabled := FALSE
	...
	RegRead, tmp, HKCU, %h_RegSubkey%, %h_Application%|%h_Class%|ci	; first look for user icon
	If ( Not ErrorLevel )
	{
	...
	Else
	{
		RegRead, tmp, HKCU, %h_RegSubkey%, %h_Application%|ci	; look for generic user icon (for app)
		If ( Not ErrorLevel )
		{
			StringSplit, tmparray, tmp, |
			h_appExe     = %tmparray1%
		...
		Else
		{	; use icon from registry-entry as default, if existing
			h_appExeIcon = 1
			RegRead, h_appExe, HKLM, SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\%h_Application%
			If ( h_appExe = "" )
			{	; otherwise do a file search
				h_appExe := f_FindFile( h_Application, A_WinDir )
				If ( h_appExe = "" )	; nothing found in windir...
					h_appExe := f_FindFile( h_Application, A_ProgramFiles )
			}
		}
	}
	f_ShowTrayIcon( h_appExe, h_appExeIcon )
...
h_ChangeIcon:
   ...
   Suspend, On
	newIconFile =
   hCIwinTitle = %lng_ChangeIconSelector%%h_MenuWin%" - %lng_WindowTitle%
	hCIcount = 1000

   If FileExist( h_appExe )
      tmpFile = %h_appExe%
   Else 
      tmpFile = %h_StarterIcon%  ; take default icon location, %A_WinDir%\system32\shell32.dll
   SetTimer, h_ChangeIcon_MoveWin, 1   ; center file selector window upon opening
	FileSelectFile, newIconFile, 1, %tmpFile% , %hCIwinTitle%, %lng_ChangeIconSelectorExt% (*.ico; *.cur; *.ani; *.exe; *.dll; *.cpl; *.scr; *.drv; *.ocx; *.vbx; *.acm; *.bpl)
	If FileExist( newIconFile )
	{
	   FileGetAttrib, tmp, %newIconFile%
	   If InStr( tmp, "D" )   ; no folders!
	      Goto, 3SuspendOff   ; outta here

      Gui, 3:Default
      Gui, +AlwaysOnTop -MaximizeBox -MinimizeBox
      Gui, Add, ListView, Section r6 w480 -Multi AltSubmit +Icon gLVevent, IconNumber
      Gui, Add, Button, ys Section w90 Disabled g3BtnOK, %lng_SetupOK%
      Gui, Add, Button, wp g3GuiClose, %lng_SetupCancel%

      ;clear list of files
      LV_Delete()
      ;list of images
      imageList := IL_Create(100, 100, 2)
      iLID := LV_SetImageList(imageList)
      If ( iLID )
         IL_Destroy(iLID)
      ...
      Loop, 9999
      {
         iconIndex := IL_Add(imageList, newIconFile, A_Index)
         If ( iconIndex > 0 ) {
            icons++
            LV_Add("Icon" . A_Index, icons)
         }
         Else
            Break
      }

      userIcon = 0   ; no icon as default
      If ( icons = 1 ) {
         userIcon = 1
         Goto, 3SetIconAndOut
      }
      Else If ( icons = 0 )   ; no icons in file!
         Goto, 3SuspendOff   ; outta here

   	hCIguiUsed := TRUE
      noDefaultBtn := TRUE
      Gui, Show, Center, %lng_ChangeIconMulti%%h_MenuWin%" - %lng_WindowTitle%
      Goto, 3SuspendOff   ; outta here
      ...
      LVevent:
         If ( A_GuiControlEvent = "I" ) {
            ...
            {  ; no icon selected, yet
               GuiControl, Enabled, %lng_SetupOK%
               GuiControl, +Default, %lng_SetupOK%
               noDefaultBtn := FALSE
            }

            rowNumber := LV_GetNext()
            If ( rowNumber )
               LV_GetText(userIcon, rowNumber, 1)
            Else {
               userIcon = 0
               GuiControl, Disable, %lng_SetupOK%
               noDefaultBtn := TRUE
            }
         }

         If ( A_GuiControlEvent = "DoubleClick" ) {
            If ( noDefaultBtn )  ; no icon selected, yet
               Return
      ...
      3BtnOK:
         If ( userIcon = 0 )
            Return
      3SetIconAndOut:
         h_appExe     := newIconFile
         h_appExeIcon := userIcon
      ...
      3SuspendOff:
         Suspend, Off
      Return   ; mandatory due to nesting!
	}
	Suspend, Off
	Return

   h_ChangeIcon_MoveWin:
      IfWinActive, %hCIwinTitle%
      {
         WinGetPos,,, hCIWidth, hCIHeight
      	WinMove,,, (A_ScreenWidth/2)-(hCIWidth/2), (A_ScreenHeight/2)-(hCIHeight/2)
      	SetTimer, h_ChangeIcon_MoveWin, Off
      }
      Else {
         hCIcount--
         If ( hCIcount = 0 )  ; switch timer always off after "hCIcount" rounds
            SetTimer, h_ChangeIcon_MoveWin, Off
      }
   Return
Return
...
;============================================================================
; function as launcher for minimizing (starter) starts here
...
h_InitStarter:
   ...
   ; number of mouse buttons
	SysGet, tmp, 43
   If ( tmp < 3 ) {
      f_ShowErrorMsg( lng_No3BtnMouseMsg )
      m2t_noMButton := TRUE
   }
   Else
		m2t_noMButton := FALSE
   ...
   f_AssignHotkey( m2t_hkKey, "h_StartNewHelper" )
   ; stealthmode key
   m2t_hkStealth =
	...
	If bkMode not in 0,1,2
	   bkMode := 0  ; 0 = Opt-Out, 1 = Opt-In, 2 = Topmost
   ; bosskey
   m2t_hkBossKey =
   ...
   ; middle mouse button
   If ( m2t_noMButton )
      m2t_hkMButton = 0 ; only 2-button-mouse found
   Else {
	   RegRead, m2t_hkMButton, HKCU, %h_RegSubkey%\Starter, HotkeyMButton
	   If ( m2t_hkMButton = 1 OR m2t_hkMButton = 2 )
	      f_AssignHotkey( "MButton", "h_StartNewHelper" )
	   Else
	      m2t_hkMButton = 1   ; default: click on title with 3rd button
   }
   ...
   ; startup minimizing
	...
	If ( ErrorLevel OR m2t_StartupTimeSpan < 1 OR m2t_StartupTimeSpan > 99999 )
		m2t_StartupTimeSpan = 25
	RegRead, m2t_StartupInterval, HKCU, %h_RegSubkey%\Starter, StartupMinInterval
	If ( ErrorLevel OR m2t_StartupInterval < 200 OR m2t_StartupInterval > 999999 )
		m2t_StartupInterval = 2500
...
h_StartNewHelper:
   If ( A_ThisHotkey = m2t_hkKey OR m2t_hkMButton = 2 ) ; what key triggered the action?
      f_StartNewHelper( "ASAP" )  ; no check for title-click
   Else
      f_StartNewHelper( "MOUSE" )
Return
...
h_SetHotkeyStarter:
   ...
   Gui, Add, Text, w250 Y+20, %lng_SetupBKTitle%
   ...
   ; on/off options
   Gui, Add, Checkbox, Section Y+20 w250 vnewBKAddMouse %tmpDis% Checked%m2t_BKAddMouse%, %lng_SetupBKAddMouse%
...
h_ButtonOK:
   ...
   m2t_hkKey := f_ComposeHotkey( newKey, addWinKeyHK, m2t_hkKey, "h_StartNewHelper", "Starter", "HotkeyMinimize", lng_SetupKey )
   ...
   ; get frontmost window's unique id
	...
	Loop, %bkWinID%
	{
      ...
      If ( Not bkReactivateID )
		...
		; 0 = opt-out mode: hide all windows, BUT user-specified ones
		; 1 = opt-in mode : hide ONLY user-specified windows
...
{  ; returns TRUE on success or the list of window IDs ("GET")
   ...
   Local h_CaptionHeight, h_BorderHeight, h_MouseX, h_MouseY, h_WindowWidth
   ...
   ; check window ID
	If ( winID ) {
      ...
      ; test class of currently active window
      If f_IsForbiddenWinClass()
         ; instead of "forbidden" window (tray, etc.)
         ; use the last active window
         SendInput, !{ESC}
		; now set "last used window"
      WinWait, A, , 1
      If ( ErrorLevel )
   ...
   Else If ( listCmd = "MOUSE" ) { ; otherwise check if middle-click on title-bar
      CoordMode, Mouse, Relative
      SysGet, h_CaptionHeight, 4	; SM_CYCAPTION
      SysGet, h_BorderHeight, 7	; SM_CXDLGFRAME
      MouseGetPos, h_MouseX, h_MouseY
      If (( h_MouseY > 0 ) And ( h_MouseY < h_CaptionHeight + h_BorderHeight )) {
         WinGetPos, , , h_WindowWidth
         If (( h_MouseX > 0 ) And ( h_MouseX < h_WindowWidth ))
            Gosub, h_suStartNewHelper1
            Return TRUE
      } Else
         MouseClick, Middle
...
{  ; return TRUE if winClass is "fobidden"
...
f_SubstEvilClass( winID, eClass="", eAppName="" )
{  ; class substitution (needed only for Evil Classes >:-|
   ; winID must be passed to this function
   ; eClass and eAppName are optional
   ; returns winID (either substituted or passed one) 
   Global h_RegSubkey

   If ( Not eClass )
      WinGetClass, eClass, ahk_id %winID%

   If ( Not eAppName )
      WinGet, eAppName, ProcessName, ahk_id %winID%

	; substitute "evil class" with "good class" if available
   eRegKey = %eAppName%|%eClass%|ec
   RegRead, goodClass, HKCU, %h_RegSubkey%, %eRegKey%
   If ( ErrorLevel Or Not goodClass )
   {  ; substitute known "evil classes"
      If InStr(eRegKey, "PSPad.exe|TfPSPad", TRUE) ; for PSPad
         goodClass = TApplication
   }

   ; get new unique window id with help of "good class" and "evil PID"
   If ( goodClass ) {
      WinGet, ePID, PID, ahk_id %winID%
      WinGet, winID, ID, ahk_pid %ePID% ahk_class %goodClass%
   }

   ; always return a unique window id
   Return winID
}
...
f_FindFile( name, startPath )
{
   StringRight, tmp, startPath, 1
   If ( ( tmp <> "" ) and ( tmp <> "\" ) )
      startPath = %startPath%\
   ; start file search
   Loop, %startPath%%name%, 0, 1
   {
      appExe = %A_LoopFileLongPath%
      IfExist, %appExe%
         Break		; exit with first found app or ""
   }
   Return %appExe%
}
...
{  ; -------------------- localization starts here --------------------
   ...
   Else If ( role = "starter" ) {
      lng_No3BtnMouseMsg			= No mouse with at 3 buttons found. Mouse support disabled!
      ...
      lng_SetupBKOpt0            = Opt-Out  -  minimize all windows EXCEPT those from BossKey list [DEFAULT]
      lng_SetupBKOpt1            = Opt-In  -  minimize ONLY windows from BossKey list
   ...
   If ( lang = "de" ) {
      ...
      Else If ( role = "starter" ) {
         lng_No3BtnMouseMsg			= Keine Maus mit 3 Knöpfen gefunden. Mausunterstützung abgeschaltet!
         ...
         lng_SetupBKOpt0            = Opt-Out  -  minimiere alle Fenster AUSSER die der BossKey-Liste [STANDARD]
         lng_SetupBKOpt1            = Opt-In  -  minimiere NUR Fenster der BossKey-Liste
   ...
   Else If ( lang = "fr" ) {
      ...
      If ( role = "helper" ) {
         ...
         lng_SetupCustomName        = Assigner un nom personnalisé
         lng_SetupCustomIcon        = Assigner une icône personnalisée.
         lng_SetupOnBKList          = Fenêtre sur la liste BossKey.
         lng_SetupShowOnTitleChange = Afficher la fenêtre quand le titre change.
         lng_SetupSOTCRegEx         = Facultatif: seulement si l'expression régulière correspond:
         lng_SetupNoMinimize        = Réduire la fenêtre immédiatement (ne pas mettre dans la barre des tâches).
         lng_SetupOnSMList          = Fenêtre sur la liste StartupMinimize.
         lng_SetupEventTitleH       = TriggerActions
         lng_SetupEvent1            = TA1: Après avoir caché la fenêtre
         lng_SetupEvent2            = TA2: Avant de cacher la fenêtre
         lng_SetupEvent3            = TA3: Avant de fermer la fenêtre
         lng_SetupEvent4            = TA4: Avant de quitter l'helper
         lng_SetupEvent5            = TA5: Après que le titre de la fenêtre ait changé
         lng_SetupEvent6            = JTA: On StartupMinimize: SEULEMENT déclencher ces actions!`nLaissez vide pour exécuter les actions spécifiés au dessus.`n(Activez StartupMinimize globallement et pour cette fenêtre également.)
      }
      Else If ( role = "starter" ) {
         ...
         lng_MenuRestoreOnly        = &Restaurer toutes les fenêtres
         lng_MenuQuitRestoreAll     = Restaurer &toutes les fenêtres && quitter
         lng_MenuQuitOnly           = &Quitter
         lng_MenuBKEditList         = Editer la liste des fenêtres &BossKey
         lng_MenuSMEditList         = Editer la liste &StartupMinimize
         lng_PurgeListHint          = Videz la zone d'édition pour trouver n'importe quelle classe ou titre de fenêtre/ID.`n`n"Titre de fenêtre" peut être un sous-ensemble du titre d'une fenêtre (c.a.d. "secret"). Des expressions régulières sont soutenues.`n"ID" doit être l'identifiant unique d'une fenêtre (c.a.d. "0xe02fe", identique aux "ahk_id" d'AHK).`nPour vérifier différents titres de fenêtre ou IDs, concatenez les avec un "|" (signe pipe).`nExample de ligne correcte: "help|0xe02fe|beat the boss|0xe02fe".
         lng_BKPurgeListTitle       = Liste des fenêtres BossKey
         lng_BKPurgeList            = Sélectionner une fenêtre à éditer ou enlever de la liste BossKey
         lng_BKPurgeListReq         = Enlever l'entrée sélectionnée de la liste BossKey?
         lng_SMPurgeList            = Sélectionner une fenêtre à éditer ou enlever de la liste StartupMinimize
         lng_SMPurgeListTitle       = Liste StartupMinimize
         lng_SMPurgeListReq         = Enlever l'entrée sélectionnée de la liste StartupMinimize?
         lng_TrayTitle              = %lng_WindowTitle%
         lng_About1                 = %lng_WindowTitle% v%versionString% (%versionDate%)
         lng_About2                 = Réduit les fenêtres dans la zone de notification de la barre des tâches sous forme d'icônes`nen appuyant sur le troisième bouton de souris ou une touche de raccourci personalisée.`nFonctionnalités supplémentaires: BossKey, Toujours au dessus, StartupMinimize,`nAgrandissement horizontal ou vertical et bien plus encore.
         lng_About3                 = Distribué sous licence GPLv3.`nCréé par Junyx / KTC^brain en juin 2005.
         lng_About4                 = Traduction française par des Demokos et des Andreone.
         lng_SetupKey               = Hotkey pour réduire la fenêtre
         lng_SetupMButtonTitle      = Utiliser le 3ème bouton de la souris pour réduire?
         ...
         lng_SetupBossKey           = BossKey
         lng_SetupBKAddMouse        = CTRL+SHIFT+3ème bouton de la souris pour ajouter/enlever une fenêtre à/de la liste BossKey. [NON]
         lng_SetupBKListToggle      = Hotkey pour ajouter/enlever`nune fenêtre à/de la liste BossKey
         lng_BKListToggleOn         = fenêtre ajoutée à la liste BossKey.
         lng_BKListToggleOff        = fenêtre enlevée de la liste BossKey.
         lng_SetupBKTitle           = Définis le mode BossKey
         lng_SetupBKOpt0            = Opt-Out  -  réduire toutes les fenêtres sauf celles de la liste BossKey [DEFAUT]
         lng_SetupBKOpt1            = Opt-In  -  réduire SEULEMENT les fenêtres de la liste BossKey
         lng_SetupBKOpt2            = Topmost  -  rduire TOUTES les fentres SAUF celle active
         lng_SetupStealthKey        = Hotkey pour StealthMode on/off
         lng_SetupAOTKey            = Hotkey pour toujours au-dessus
         lng_SetupYMaxKey           = Hotkey pour agrandir verticallement
         lng_SetupXMaxKey           = Hotkey pour agrandir horizontallement
         lng_SetupNoBKey            = Hotkey pour la fonction NoButtons
         lng_SetupAOTSlider1        = Transparence pour toujours au-dessus [NON]:
         lng_SetupAOTSlider2        = Opaques
         lng_SetupAOTSlider3        = Transparent
         lng_SetupStartupTitle      = StartupMinimize
         lng_SetupStartupEnable     = StartupMinimize: autorise la réduction de certaines fenêtres au démarrage de Min2Tray. Nécessite un redémarrage. [NON]
         lng_SetupStartupTS1        = Durée:
         lng_SetupStartupTS2        = sec
         lng_SetupStartupIN1        = Intervalle:
         lng_SetupStartupIN2        = ms
         lng_SetupStartupHint       = Vérifier toutes les X milliseconds (intervalle)`nsi une fenêtre ouverte doit être réduite.`nStartupMinimize sera désactivé`nlosque X secondes (durée) se seront écoulées.
         lng_SetupEventEnable       = Activer globalement les TriggerActions. [NON]
         lng_SetupEventTitleBK      = TriggerActions pour BossKey
         lng_SetupEventBK1          = BK1: Avant que la BossKey réduise les fenêtres
         lng_SetupEventBK2          = BK2: Après que la BossKey ait réduit les fenêtres
      }
      Else {
         lng_AOTtraytip             = Fenêtre en mode toujours au-dessus.
         lng_Postraytip             = position personalisée assignée.
         lng_AMaxtraytip            = la fenêtre est maintenant agrandie.
         lng_AMaxNottraytip         = la fenêtre ne peut être agrandie.
         lng_YMaxtraytip            = Fenêtre maximisée verticalement.
         lng_YMaxNottraytip         = La fenêtre ne peut être maximisée verticalement.
         lng_XMaxtraytip            = la fenêtre est maintenant agrandie horizontallement.
         lng_XMaxNottraytip         = la fenêtre ne peut être agrandie horizontallement.
         lng_NoBtraytip             = tous les boutons sont retirés de la barre de titre.
         lng_ExitWithOSError        = Erreur: Système d'exploitation incompatible. Nécessite Windows NT ou supérieur !
         lng_ExitWithVersionError   = Erreur: Nécessite AutoHotkey.exe version %h_MinReqAHK% ou supérieur !
         lng_WindowTitle            = Min2Tray
         lng_SetupOK                = &Appliquer
         lng_SetupCancel            = &Annuler
         lng_SetupRemove            = &Retirer
         lng_SetupEdit              = &Editer
         lng_SetupClose             = &Fermer
         lng_MenuPrefs              = &Préférences...
         lng_SetupTitle             = Prefs - %lng_WindowTitle%
         lng_SetupHint              = Astuce : Cliquez sur Suppr sur le clavier et confirmez en cliquant "%lng_SetupOK%" pour supprimer une hotkey !
         ...
         lng_SetupEventError        = L'action suivante contient une erreur, corrigez la ou supprimez la SVP :
         lng_SetupEventHint1        = Pour déclencher plusieurs actions, concaténez les avec un "|" (signe pipe).`nSupprimez une action en effaçant sa zone d'édition.`n`nLes TriggerActions sont:
         lng_SetupEventHint2        =
         ( LTrim
            key1:<touches à envoyer>
            `t- activer la fenêtre et envoyer des touches simulées
            `t- caractères speciaux: # = winkey; ^ = CTRL; ! = ALT; + = SHIFT
            key2:<temporisation,touches à envoyer>
            `t- activer la fenêtre et envoyer des touches simulées
            `t- vous devez précisier un "temps de frappe" (en millisecondes) 
            key4:<touches à envoyer>
            `t- envoyer des touches simulées à une fenêtre, sans l'activer
            `t- à utiliser pour envoyer des caractères à "cmd.exe"
            key5:<contrôle,touches à envoyer>
            `t- envoyer des touches simulées à un contrôle d'une fenêtre
            `t- c.a.d. pour "notepad.exe": key5:Edit1,Ceci est du texte dans notepad.
            key6:<temporisation,contrôle,touches à envoyer>
            `t- envoyer des touches simulées à un contrôle d'une fenêtrew
            `t- vous devez précisier un "temps de frappe" (en millisecondes) 
            act1:<activer le fenêtre>
            `t- activer la fenêtre en la mettant aupremier plan
            `t- utile après l'exécution de key4, key5 ou key6
            act2:<re-activate last activate window>
            `t- bring last activate and remembered window to front
            `t- i.e. after un-hiding BossKey-minimized windows
            aot1:<mettre la fenêtre au premier plan>
            `t- les seuls paramètres acceptés sont:
            `t   ON = mettre la fenêtre au premier plan
            `t   OFF = retirer la fenêtre du premier plan
            nob1:<Mode NoButtons>
            `t- les seuls paramètres acceptés sont:
            `t   ON = enlever les boutons (SysMenu) de titlebar de la fenêtre
            `t   OFF = montrer les boutons encore
            max1:<Agrandir la fenêtre>
            `t- les seuls paramètres acceptés sont:
            `t   X = agrandir la fenêtre horizontallement
            `t   Y = agrandir la fenêtre verticallement
            `t   BOTH = agrandir la fenêtre dans les deux directions
            pos1:<X,Y,W,H>
            `t- changer la position et/ou la taille d'une fenêtre:
            `t   X,Y = coin haut gauche en pixels
            `t   W,H = largeur et hauteur in pixels
            `t- toutes les valeurs sont facultatives (mettez les virgules!)
            `t- les valeurs vides ne sont pas modifiés, c.a.d.:
            `t   20,,,400 = X est 20p depuis la gauche et hauteur est 400p 
            `t   50,50 = positionner le coin haut gauche à X=50p et Y=50p 
            `t   ,,800,200 = changer la taille de la fenêtre à 800 x 200 pixels 
            aud1:<changer le son principal>
            `t- les seuls paramètres acceptés sont:
            `t   MUTE_ON = couper le son principal
            `t   MUTE_OFF = annuler couper le son
            `t   MUTE_TOGGLE = permuter entre MUTE_ON et MUTE_OFF
            snd1:<chemin complet du fichier>
            `t- jouer un fichier son (*.wav)
            `t- attendre qu'il se termine
            snd2:<chemin complet du fichier>
            `t- jouer un fichier son (*.wav)
            `t- ne pas attendre qu'il se termine
            run1:<chemin complet du fichier>
            `t- exécuter un programme
            `t- ne pas attendre qu'il se termine
            run2:<chemin complet du fichier>
            `t- exécuter un programme (et essayer de cacher sa fenêtre)
            `t- ne pas attendre qu'il se termine
            run3:<chemin complet du fichier>
            `t- exécuter un programme, ne pas attendre qu'il se termine
            `t- la dernière ligne de commande paramètre est ahk_id de fenêtre
            rnw1:<chemin complet du fichier>
            `t- exécuter un programme
            `t- attendre qu'il se termine
            rnw2:<chemin complet du fichier>
            `t- exécuter un programme (et essayer de cacher sa fenêtre)
            `t- attendre qu'il se termine
            rnw3:<chemin complet du fichier>
            `t- exécuter un programme, attendre qu'il se termine
            `t- la dernière ligne de commande paramètre est ahk_id de fenêtre
            msg1:<n'importe quel texte>
            `t- afficher une boîte de message avec du texte
            `t- la fenêtre a un bouton OK
            slp1:<délai>
            `t- durée de la pause (en millisecondes)
            `t- entre 0 et 2147483647 (24 jours)
            min1:<réduire cette fenêtre>
            `t- arrêter les prochaines actions et
            `t- réduire cette fenêtre immédiatement
            stm1:<manipuler StealthMode>
            `t- les seuls paramtres accepts sont:
            `t   ON = activer StealthMode (ne montre aucun icne)
            `t   OFF = dsactiver StealthMode
            `t   TOGGLE = permuter le mode entre ON et OFF
            reg1:<type,subkey,name,value>
            `t- crire une valeur dans la base de registre:
            `t   type: REG_SZ, REG_DWORD, REG_BINARY, REG_EXPAND_SZ
            `t         or REG_MULTI_SZ
            `t   subkey: le nom de la clef sous HKEY_CURRENT_USER, c.a.d. Software\MyApp
            `t   name: le nom de la valeur  crire, peut tre laisse vide pour "(Default)"
            `t   value: la valeur  crire
            reg2:<subkey,name>
            `t- supprimer une valeur de la base de registre:
            `t   subkey: le nom de la clef sous HKEY_CURRENT_USER, c.a.d. Software\MyApp
            `t   name: le nom de la valeur  supprimer
            cfw1:<changer le timer CheckForWin>
            `t- USAGE INTERNE SEULEMENT! peut empêcher le bon fonctionnement!
            `t- valide seulement pour les TriggerAction des fenêtres réduites, pas BossKey
            `t- le timer vérifie périodiquement l'état de la fenêtre
            `t- les seuls paramètres acceptés sont:
            `t   OFF = mettre en pause le timer CheckForWin
            `t   ON = réactiver le timer (A TOUJOURS FAIRE!)
         )
      }
   }
