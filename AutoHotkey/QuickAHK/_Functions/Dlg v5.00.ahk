...
/*
 ...
 Remarks:
				Although picture shows differently, Replace dialog doesn't have options for replace direction for unknown reasons. The flag will contain "d" always.
 */ 
...
Dlg_callback(wparam, lparam, msg, hwnd) {
	...
	if (hFlags & FR_REPLACE) || (hFlags & FR_REPLACEALL) {
		...
		return %handler%(event, flags "d", findText, replaceText)
	}
}
