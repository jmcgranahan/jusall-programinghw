...
;       (usually 56) to 99 range. If an available window cannot be found, an
;       error message is displayed.
;
;       Important Note:  Although this function can theoretically create up to
;       99 independent windows, creating more than one InfoGUI window at a time
;       is not recommended because the Owner GUI window is automatically
;       re-enabled when a InfoGUI window is closed.  When this occurs, any
;       InfoGUI window still open becomes modeless (non-modal).  
;   
;
;
;   Customizing
...
InfoGUI(p_Owner=""
    ...
    loop
        ...
        if l_GUI=99
            {
            MsgBox 262160,InfoGUI Error,
               (ltrim join`s
                Unable to create %A_ThisFunc% window.
                GUI windows %s_GUI% to 99 are already in use.  %A_Space%
               )

            outputdebug
                ,End Func: %A_ThisFunc% (Unable to create window.  Return=FAIL)
            return FAIL
            }
    ...
    loop
        {
        sleep 100
        IfWinNotExist ahk_id %l_InfoGUI_hWnd% 
            break
        }
