...
;-- Highlighting file for the HiEdit control provided by skwire
;**********************************************
;*                                            *
;*                                            *
;*                                            *
...
WM_RBUTTONUP(wParam,lParam,msg,hWnd)
    {
    ;-- Note: Value of wParam is ignored for now
...
;*                     *
;***********************
HE_BlockComment(p_hEdit,p_Cmd="",p_BCChars="")
    {
    ;[=============]
    ;[  Intialize  ]
    ;[=============]
    ...
    HE_GetSel(p_hEdit,l_StartSelPos,l_EndSelPos)


    ;-- Get position of the beginning of the line
    l_FirstCharPos:=HE_LineIndex(p_hEdit,HE_LineFromChar(p_hEdit,l_StartSelPos))


    ;-- Reselect if the user selection does not start at the beginning of the line
    if (l_FirstCharPos<>l_StartSelPos)
        HE_SetSel(p_hEdit,l_FirstCharPos,l_EndSelPos)


    ;-- Get selected text
    l_SelectedText:=HE_GetSelText(p_hEdit)
    ...
    if p_Cmd in Insert,Add
        ...
        if StrLen(l_SelectedText)=0
            ...
            {
            ;-- Set capacity for l_NewText
            VarSetCapacity(l_NewText,round(StrLen(l_SelectedText)*1.15))


            ;-- Is last character a NL?
            l_LastChar:=""
            if SubStr(l_SelectedText,StrLen(l_SelectedText))="`n"
                {
                l_LastChar:="`n"
            ...
            loop parse,l_SelectedText,`n,`r
                ...
                if StrLen(l_NewText)=0
                    ...
                    l_NewText=%l_NewText%`n%p_BCChars%%A_LoopField%
                }


            ;-- Append last char if needed
            if StrLen(l_LastChar)
                l_NewText=%l_NewText%%l_LastChar%
            }


        ;-- Replace selected text with l_NewText
        ErrorLevel:=HE_ReplaceSel(p_hEdit,l_NewText)
        ...
        ;-- Reselect text
        HE_SetSel(p_hEdit,l_NewStartSelPos,l_NewEndSelPos)
        }
     else
        {
        ;-----------------------
        ...
        if StrLen(l_SelectedText)=0
            ...
            if (l_StartSelPos<=(HE_GetTextLength(p_hEdit)-StrLen(p_BCChars)))
                ;-- Look forward to see if the first characters are at the
                ;   beginning of the a comment block
                if HE_GetTextRange(p_hEdit,l_StartSelPos,l_StartSelPos+StrLen(p_BCChars))=p_BCChars
                    ;-- Reselect and recapture
                    HE_SetSel(p_hEdit,l_StartSelPos,l_StartSelPos+StrLen(p_BCChars))
                    l_SelectedText:=HE_GetSelText(p_hEdit)
            }
        ...
        if StrLen(l_SelectedText)
            {
            ;-- Set capacity for l_NewText
            VarSetCapacity(l_NewText,StrLen(l_SelectedText))
            ...
            loop parse,l_SelectedText,`n
                ...
                if A_Index=1
                    ...
                    l_NewText=%l_NewText%`n%l_LoopField%
                }
            }
...
;*                  *
;********************
HE_ShiftLine(p_hEdit,p_Cmd="",p_Chars="")
    {
    ;[=============]
    ;[  Intialize  ]
    ;[=============]
    ...
    if p_Cmd in Insert,Add
        ...
        if StrLen(l_SelectedText)=0
            ...
            if l_AllSpaces
                StringTrimRight
                    ,l_NewText
                    ,l_NewText
                    ,% Mod(l_StartSelPos-HE_LineIndex(p_hEdit,-1),StrLen(l_NewText))
            ...
            {
            ;-- Set capacity for l_NewText
            VarSetCapacity(l_NewText,round(StrLen(l_SelectedText)*1.25))
            ...
            ;-- Single line?
             if InStr(l_SelectedText . l_LastChar,"`n")=0
            and HE_GetLine(p_hEdit)<>l_SelectedText
                {
                l_NewText:=p_Chars
                l_Reselect:=false
                }
                ...
                loop parse,l_SelectedText,`n,`r
                    ...
                    if StrLen(l_NewText)=0
                        ...
                        l_NewText=%l_NewText%`n%p_Chars%%A_LoopField%
                    }
                }
        ...
        if StrLen(l_SelectedText)=0
            ...
            l_StartLinePos:=HE_LineIndex(p_hEdit,-1)
            if (l_StartSelPos>l_StartLinePos)
                {
                ;-- Get the line in question
                l_CurrentLine:=HE_GetLine(p_hEdit,-1)
                ...
                if (l_NewStartSelPos<>l_StartSelPos)
                    ...
                    HE_SetSel(p_hEdit,l_StartSelPos,l_EndSelPos)
    ...
    if l_Reselect and StrLen(l_SelectedText)
        HE_SetSel(p_hEdit,l_StartSelPos,l_StartSelPos+StrLen(l_NewText))
...
;;;;;          ,PD_CURRENTPAGE       :=0x00400000    ;-- Windows 2000+
;;;;;          ,PD_NOCURRENTPAGE     :=0x00800000    ;-- Windows 2000+
...
RPBuildTable()
    {
    ;-- Global variables
    Global $MaxRestorePoints
          ,$RPDir


    ;-- Initialize
    VarSetCapacity(l_RestoreTable,$MaxRestorePoints*14)
    ...
    loop %$RPDir%\*.ahk
        ...
        if StrLen(l_RestoreTable)=0
            ...
            l_RestoreTable=%l_RestoreTable%|%l_RestoreTimeStamp%
        }
...
RPCreate(p_CheckForDup=true)
    ...
    IfExist %l_RPFile%
        return
            ;-- Note: A "duplicate" restore point file may exist if the user
            ;   submits multiple requests within a second
    
    
    ;-- Save current workspace to restore point file
    HE_SaveFile($QAHKGUI_hEdit,l_RPFile)
    if ErrorLevel
        {
        gui +OwnDialogs
        MsgBox
    ...
    loop parse,p_RunPIDList,`,
        ...
        if ErrorLevel
            if StrLen(l_RunPIDList)=0
                ...
                l_RunPIDList:=l_RunPIDList . "," . A_LoopField
        }
...
if StrLen($BlockComment)=0
    ...
    ;-- Remove borders
    $BlockComment:=SubStr($BlockComment,2,StrLen($BlockComment)-2)
...
if $ToolbarButtons is not Space
    {
    ;-- Remove borders
    $ToolbarButtons:=SubStr($ToolbarButtons,2,StrLen($ToolbarButtons)-2)
...
if StrLen($RunParms)=0
    ...
    {
    ;-- Remove borders
    $RunParms:=SubStr($RunParms,2,StrLen($RunParms)-2)
...
if StrLen($RunParmsDDL)
    {
    ;-- Remove borders
    $RunParmsDDL:=SubStr($RunParmsDDL,2,StrLen($RunParmsDDL)-2)
...
if StrLen($NewScript)=0
    ...
    {
    ;-- Remove borders
    $NewScript:=SubStr($NewScript,2,StrLen($NewScript)-2)
...
if StrLen($DebugScript)=0
    ...
    {
    ;-- Remove borders
    $DebugScript:=SubStr($DebugScript,2,StrLen($DebugScript)-2)
...
if SubStr($CLParameter,StrLen($CLParameter))=""""
...
FileSetAttrib -RSH,%$CLParameter%


;-- Update status bar
$Message=Contents of "%$CLParameter%" copied to the workspace.
SB_SetText($Message)
...
IfNotExist %$EditDir%\.
    ...
    if ErrorLevel
        ...
        outputdebug,
           (ltrim join`s
            End Subrou: %A_ThisLabel% -
            Unable to create "Edit" folder.  Program stopped.
           )
...
IfNotExist %$EditFile%
    ...
    if ErrorLevel
        ...
        outputdebug,
           (ltrim join`s
            End Subrou: %A_ThisLabel% -
            Unable to create the initial workspace file.  Program stopped.
           )
...
if not ErrorLevel
    ...
    outputdebug,
       (ltrim join`s
        End Subrou: %A_ThisLabel% - 
        Unable to open workspace file.  Program stopped.)
       )
...
;-- Bounce if PageSetupDlg or PrintDlg have been called before
if hDevMode
    return
...
QAHKGUI_OnFind(p_Event,p_Flags,p_FindWhat,Dummy="")
    ...
    if p_Flags contains d
        l_FindPos:=HE_FindText($QAHKGUI_hEdit,p_FindWhat,l_EndSelPos,-1,l_Flags)
     else
        l_FindPos:=HE_FindText($QAHKGUI_hEdit,p_FindWhat,l_StartSelPos,0,l_Flags)


    ;-- If necessary, convert l_FindPos to signed integer
    if l_FindPos>0x7FFFFFFF  ;-- 0x7FFFFFFF=2147483647=largest possible 32-bit signed integer value
        l_FindPos:=-(~l_FindPos)-1
...
;*                   *
;*********************
QAHKGUI_ConvertCase(p_hEdit,p_Case)
    {
...
;-- Update status bar
$Message=Contents of "%$DropFile%" copied to the workspace.
SB_SetText($Message)
SetTimer QAHKGUI_ClearStatusBar,25000
...
;-- Turn off resizing during this exercise 
$Resize:=false
...
;[===================]
gui %$QAHKGUI%:+OwnDialogs
InputBox $GoToLine,Go To...,Line number:, ,130,120
if ErrorLevel
	return
...
;-- Update status bar
$Message=Contents of "%$QAHKGUI_CopyFromFileName%" copied to the workspace.
SB_SetText($Message)
SetTimer QAHKGUI_ClearStatusBar,25000
...
MsgBox
    ...
    ,External Editor,
       (ltrim join`s
        ...
        you've finished making changes, save the file, close %t_EditorName2%,
...
;-- Update status bar
SB_SetText("Workspace saved to " . $QAHKGUI_SaveToFileName)
SetTimer QAHKGUI_ClearStatusBar,25000
...
HE_SetSel($QAHKGUI_hEdit,0,-1)
;;;;;HE_LineScroll($QAHKGUI_hEdit,0,999999)  ;-- Scroll to the bottom
    ;-- Note: This command selects all lines but does not scroll to the bottom
    ;   of the control.  It does not match the behavior of the Ctrl+A keyboard
    ;   shortcut but it is consistent with the actions of the built-in context
    ;   menu.
    ;
    ;   OK, the jury is still out on this one.
...
;-- Flags
NumPut(PSD_MARGINS,PAGESETUPDLG_Structure,16,"UInt")            ;-- Flags
...
and SubStr($OptionsGUI_ExtEditorPath,StrLen($OptionsGUI_ExtEditorPath))=""""
    $OptionsGUI_ExtEditorPath:=SubStr($OptionsGUI_ExtEditorPath,2,StrLen($OptionsGUI_ExtEditorPath)-2)
...
and SubStr($OptionsGUI_AutoHotkeyPath,StrLen($OptionsGUI_AutoHotkeyPath))=""""
    $OptionsGUI_AutoHotkeyPath:=SubStr($OptionsGUI_AutoHotkeyPath,2,StrLen($OptionsGUI_AutoHotkeyPath)-2)
...
;   change their minds and change it back.  Superfluous RP files will
...
IfNotExist %$RPDir%\*.ahk
    {
    gui +OwnDialogs  
    MsgBox
        ,48     ;-- 48 = 0 (OK button) + 48 ("!" icon)
        ,Restore
        ,There are no restore points to select from.  %A_Space%
...
;-- Identify window handle
gui %$RPSelectGUI%:+LastFound
WinGet $RPSelectGUI_hWnd,ID
GroupAdd $RPSelectGUI_Group,ahk_id %$RPSelectGUI_hWnd%


;-- GUI options
gui Margin,6,6
gui +LabelRPSelectGUI_
    || +Resize
    || -MinimizeBox
    || +MinSize
...
gui Add
   ,GroupBox
   ,xm ym w220 h165
        || hWnd$RPSelectGUI_RPListGB_hWnd
        || v$RPSelectGUI_RPListGB
   ,Restore Point


gui Add
   ,ListView
   ,xp+10 yp+20 w200 h140
        || Section
        || +AltSubmit
        || Count999  ;-- 999=Maximum number of restore points
        || -Hdr
        || -Multi
        || hWnd$RPSelectGUI_RPList_hWnd
        || v$RPSelectGUI_RPList
        || gRPSelectGUI_RPList
   ,Display Name|Path
...
gui Add
   ,GroupBox
   ,xm+220 ym w330 h200
        || hWnd$RPSelectGUI_ViewerGB_hWnd
        || v$RPSelectGUI_ViewerGB
   ,Viewer


;-- HiEdit control
$RPSelectGUI_hEdit:=HE_Add($RPSelectGUI_hWnd,235,25,310,170,"HSCROLL VSCROLL HILIGHT",$LibDir . "\HiEdit.dll")


;-- Font, font size, font style
HE_SetFont($RPSelectGUI_hEdit,"s" . $RPViewerFontSize . A_Space . $RPViewerFontStyle . "," . $RPViewerFont)


;-- Line numbers bar?
if $RPViewerLineNumbersBar
    HE_LineNumbersBar($RPSelectGUI_hEdit,"autosize",30,10)
...
gui Add
   ...
   ,xm y180 w70 h25
        || +Disabled
        || hWnd$RPSelectGUI_SelectButton_hWnd
        || v$RPSelectGUI_SelectButton
        || gRPSelectGUI_SelectButton
   ,&Select

gui Add
   ,Button
   ,x+5 wp hp
        || hWnd$RPSelectGUI_CancelButton_hWnd
        || v$RPSelectGUI_CancelButton
        || gRPSelectGUI_Close
   ,&Cancel
...
loop %$RPDir%\*.ahk
    ...
    LV_Add("",$RPDisplayField,A_LoopFileLongPath)
    }
...
;-- Attach GUI objects
Attach($RPSelectGUI_RPListGB_hWnd    ,"h")
Attach($RPSelectGUI_RPList_hWnd      ,"h")
Attach($RPSelectGUI_ViewerGB_hWnd    ,"w h")
Attach($RPSelectGUI_hEdit            ,"w h")
Attach($RPSelectGUI_SelectButton_hWnd,"y")
Attach($RPSelectGUI_CancelButton_hWnd,"y")
...
;[============]
;-- Render but don't display
gui Show,Hide,Select Restore Point


;-- Calculate X/Y and Show it
PopupXY($QAHKGUI,$RPSelectGUI,$PosX,$PosY)
gui Show,x%$PosX% y%$PosY%
...
;*                               *
;*********************************
RPSelectGUI_ToggleLineNumbersBar:

if $RPViewerLineNumbersBar
    {
    HE_LineNumbersBar($RPSelectGUI_hEdit,"hide",0,0)
    $RPViewerLineNumbersBar:=false
    }
 else
    {
    HE_LineNumbersBar($RPSelectGUI_hEdit,"autosize")
...
if A_GuiEvent=DoubleClick
    ...
    if LV_GetCount("Selected")
        gosub RPSelectGUI_SelectButton
...
GUIControl Enable,$RPSelectGUI_SelectButton
...
;-- Close current file
HE_CloseFile($RPSelectGUI_hEdit)
...
if $FileSize<=10485760  ;-- 10 MB
    {
    HE_OpenFile($RPSelectGUI_hEdit,$RPFile)
    if not ErrorLevel
        {
        ;-- Disable "Select" button
        GUIControl Disable,$RPSelectGUI_SelectButton
    ...
    {
    ;-- Read RPFile
    FileRead $RPSelectGUI_Viewer,*m524288 %$RPFile%  ;-- 524288=512K
    if ErrorLevel
        {
        ;-- Disable "Select" button
        GUIControl Disable,$RPSelectGUI_SelectButton
    ...
    ;-- Load to Viewer
    HE_SetSel($RPSelectGUI_hEdit,0,-1)
    HE_ReplaceSel($RPSelectGUI_hEdit,$RPSelectGUI_Viewer)
    
    
    ;-- Empty undo buffer
    HE_EmptyUndoBuffer($RPSelectGUI_hEdit)
    
    
    ;-- Scroll to the top/left
    HE_LineScroll($RPSelectGUI_hEdit,-9999999,-9999999)
    }
...
;-- Shutdown RPSelectGUI
gosub RPSelectGUI_Exit
...
;-- Update status bar
SB_SetText("Workspace restored from " . $RPFile)
SetTimer QAHKGUI_ClearStatusBar,25000
...
if LV_GetCount()
    ...
    LV_Modify($Row,"+Select +Vis")
        ...
        ;   RPSelectGUI_RPList routine which will populate the RP viewer with
...
;*******************************
;-- Begin #IfWinActive directive
#IfWinActive ahk_group $RPSelectGUI_Group
...
if $Control<>$RPSelectGUI_RPList
    return


;-- Delete it
gosub RPSelectGUI_Delete
return
...
;[================]
^WheelUp::
gosub RPSelectGUI_IncreaseFontSize
return
...
;[==================]
^WheelDown::
gosub RPSelectGUI_DecreaseFontSize
return
...
^Numpad0::
^NumpadIns::
gosub RPSelectGUI_DefaultFontSize
return
...
;[==========]
^l::
gosub RPSelectGUI_ToggleLineNumbersBar
return
...
gui Add
   ,DropDownList
   ,xp+10 yp+20 w315 h20 r7 Choose1
